...
def add_transaction(user: dict, tx_type: str, amount: float, currency: str, wallet_id: int, category_id: int, comment=''):
    ...
    with engine.begin() as conn:
        ...
        # Concurrency-safe balance update: avoid "read-check-write" race conditions.
        if tx_type == 'expense':
            updated = conn.execute(
                update(wallets)
                .where(and_(
                    wallets.c.id == wallet_id,
                    wallets.c.family_id == family_id,
                    wallets.c.balance >= amount,
                ))
            ...
            conn.execute(
                update(wallets)
                .where(and_(wallets.c.id == wallet_id, wallets.c.family_id == family_id))
                .values(balance=wallets.c.balance + amount)
            )
...
def transfer_between_wallets(user: dict, from_wallet_id: int, to_wallet_id: int, amount_from: float, amount_to: float | None = None, comment=''):
    ...
    with engine.begin() as conn:
        ...
        now = _now()
        # Concurrency-safe withdrawal: atomic guard prevents overspend under parallel transfers.
        updated = conn.execute(
            update(wallets)
            .where(and_(
                wallets.c.id == from_wallet_id,
                wallets.c.family_id == family_id,
                wallets.c.balance >= amount_from,
            ))
            .values(balance=wallets.c.balance - amount_from)
        )
        if not updated.rowcount:
        ...
        deposited = conn.execute(
            update(wallets)
            .where(and_(wallets.c.id == to_wallet_id, wallets.c.family_id == family_id))
            .values(balance=wallets.c.balance + amount_to)
        )
        if not deposited.rowcount:
            raise ValueError('Кошелек-получатель не найден')
