...
self.addEventListener('fetch', (event) => {
  ...
  // Runtime caching: only allowlist same-origin WebApp assets to avoid unbounded cache growth.
  if (url.origin !== self.location.origin || !url.pathname.startsWith('/webapp/')) {
    event.respondWith(fetch(req).catch(() => caches.match('/webapp/index.html')));
    return;
  }
  event.respondWith(
    caches.match(req).then((cached) => cached || fetch(req).then((res) => {
      // Cache only successful, non-opaque responses. Skip huge responses to reduce quota risk.
      try {
        const okToCache = res && res.ok && res.type !== 'opaque' && res.status === 200;
        const len = Number(res.headers.get('content-length') || 0);
        if (okToCache && (!len || len <= 2_000_000)) {
          const copy = res.clone();
          caches.open(CACHE_NAME).then((cache) => cache.put(req, copy));
        }
      } catch (_) {}
      return res;
    }).catch(() => caches.match('/webapp/index.html')))
  );
});
