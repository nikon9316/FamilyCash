(() => {
  ...
  async function request(path, { method = 'GET', body = null, idempotent = false, idempotencyKey = null } = {}) {
    const headers = authHeaders();
    if (body !== null) headers['Content-Type'] = 'application/json';
    const key = idempotencyKey || (idempotent ? idem() : null);
    ...
    try {
      ...
      if (window.FFOffline?.isQueueable?.(method, path) && (!navigator.onLine || String(e.message || '').includes('Failed to fetch'))) {
        const queued = await window.FFOffline.enqueue({ path, method, body, idempotency_key: key || undefined });
