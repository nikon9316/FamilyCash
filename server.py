...
async def api_transaction(request):
    try:
        ...
        return json_error('Ошибка сохранения операции. Подробности записаны в лог.', 500)
...
async def api_scheduled_payment_issues(request):
    try:
        ...
        logger.exception('Scheduled payment issues failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось загрузить список ошибок автоплатежей.'}, status=500)
...
async def api_month_end_money(request):
    try:
        ...
        logger.exception('Month end money failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось рассчитать остаток до конца месяца.'}, status=500)
...
async def api_mandatory_payment_pay(request):
    try:
        ...
        logger.exception('Mandatory payment pay failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось оплатить обязательный платеж.'}, status=500)
...
async def api_mandatory_payment_mark_paid(request):
    try:
        ...
        logger.exception('Mandatory payment mark paid failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось отметить обязательный платеж.'}, status=500)
...
async def api_mandatory_payment_linkable_transactions(request):
    try:
        ...
        logger.exception('Mandatory linkable transactions failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось загрузить список операций для привязки.'}, status=500)
...
async def api_mandatory_payment_link_existing(request):
    try:
        ...
        logger.exception('Mandatory link existing failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось привязать операцию.'}, status=500)
...
async def api_scheduled_payment_issue_resolve(request):
    try:
        ...
        logger.exception('Resolve scheduled issue failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось скрыть ошибку как решенную.'}, status=500)
...
async def api_scheduled_payment_retry(request):
    try:
        ...
        logger.exception('Retry scheduled payment failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось повторить автоплатеж.'}, status=500)
...
async def api_scheduled_payment_disable_auto(request):
    try:
        ...
        logger.exception('Disable scheduled auto-create failed')
        _capture_exception(exc)
        return web.json_response({'ok': False, 'error': 'Не удалось отключить автосписание.'}, status=500)
