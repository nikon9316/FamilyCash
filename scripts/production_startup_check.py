...
async def main() -> int:
    ...
    try:
        ...
        # Local dev environments may not have optional report/export deps installed.
        if e.name in ('openpyxl', 'reportlab'):
            raise RuntimeError(
                f'Missing optional dependency: {e.name}. '
                'Install requirements.txt to run full production startup check.'
            ) from e
        raise
